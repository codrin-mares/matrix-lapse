...
const processFinalAxis = (line: JSX.Element[], type: 'horizontal' | 'vertical'): JSX.Element[] => {
  ...
  const firstItem = <div key={'first'} className="text-secondary join-item h-8 w-8 flex items-center justify-center" />;
  const lastItem = <div key={'last'} className="text-secondary join-item h-8 w-8 flex items-center justify-center" />;
...
const PositionAxis = ({ len, type, isZeroBased }: Props): JSX.Element => {
  const line = Array.from(Array(len)).map((_, i) => (
    <div key={i} className="text-secondary join-item h-8 w-8 flex items-center justify-center">
      {isZeroBased ? i : i + 1}
    </div>
  ));
